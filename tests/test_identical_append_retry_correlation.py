import pytest

from distlab.raft import AppendEntriesResponse, LogEntry, RaftCluster, RaftRole
from distlab.replication import LeaderReplicator, ReplicationResponseMissing
from distlab.simulator import FaultAction, FaultPlan, FaultRule, Simulator


def test_delayed_identical_failure_cannot_satisfy_current_append_retry() -> None:
    sim = Simulator()
    cluster = RaftCluster(sim, ("n1", "n2", "n3"))
    leader = cluster.node("n1")
    leader.start_election()
    sim.run()
    assert leader.role is RaftRole.LEADER

    leader._persist_log(
        (
            LogEntry(term=leader.current_term, command="a"),
            LogEntry(term=leader.current_term, command="b"),
        )
    )
    replicator = LeaderReplicator(leader)

    sim.send(
        "n2",
        "n1",
        AppendEntriesResponse(
            term=leader.current_term,
            follower_id="n2",
            success=False,
            match_index=0,
        ),
        delay=10,
    )
    stale_send = next(
        record
        for record in reversed(sim.trace)
        if record.kind == "send"
        and record.details["src"] == "n2"
        and record.details["dst"] == "n1"
        and isinstance(record.details["payload"], AppendEntriesResponse)
    )
    stale_ordinal = int(stale_send.details["ordinal"])
    sim.fault_plan = FaultPlan(
        rules=(
            FaultRule(
                FaultAction.DROP,
                src="n2",
                dst="n1",
                ordinal=stale_ordinal + 1,
            ),
        )
    )

    with pytest.raises(ReplicationResponseMissing, match="no AppendEntries response"):
        replicator.replicate("n2", max_attempts=1)

    observed = [
        record
        for record in sim.trace
        if record.kind == "raft-append-entries"
        and record.details["leader"] == "n1"
        and record.details["follower"] == "n2"
        and record.details["prev_log_index"] == 2
        and record.details["entry_count"] == 0
        and record.details["success"] is False
    ]
    assert observed
    delayed = [
        record
        for record in sim.trace
        if record.kind == "raft-append-response"
        and record.details["follower"] == "n2"
        and record.details["success"] is False
        and record.details["match_index"] == 0
    ]
    assert delayed

    progress = replicator.progress("n2")
    assert progress.match_index == 0
    assert progress.next_index == 3
    assert not [
        record
        for record in sim.trace
        if record.kind == "raft-replication-backtrack"
        and record.details["follower"] == "n2"
    ]

    sim.fault_plan = FaultPlan()
    assert replicator.replicate("n2", max_attempts=3) is True
    assert replicator.progress("n2").match_index == 2
    assert replicator.progress("n2").next_index == 3

    cluster.assert_log_matching()
