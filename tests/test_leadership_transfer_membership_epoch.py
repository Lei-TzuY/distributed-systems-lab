...
def _leader_cluster() -> tuple[Simulator, ReconfigurableRaftCluster]:
    sim = Simulator()
...
def _assert_transfer_rejected_without_election(
    sim: Simulator,
...
def test_timeout_now_is_rejected_after_voting_configuration_changes() -> None:
    ...
    _assert_transfer_rejected_without_election(sim, cluster, attempt_id=attempt_id)
    harness.checkpoint()


def test_timeout_now_is_rejected_after_configuration_changes_away_and_back() -> None:
    sim, cluster = _leader_cluster()
    transport = LeadershipTransferTransport.for_cluster(cluster)
    harness = RaftSafetyHarness(cluster)
    harness.checkpoint()
