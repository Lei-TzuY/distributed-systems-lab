...
def test_transfer_reports_retry_catchup_budget_exhaustion(monkeypatch) -> None:
    sim = Simulator(
    ...
    monkeypatch.setattr(LeaderReplicator, "recover_peer", exhaust_retry_recovery)

    with pytest.raises(
        LeadershipTransferIncomplete,
        match="leadership transferee 'n2' did not re-catch up",
    ):
    ...
    assert failures[-1].details["reason"] == "replication attempt budget exhausted"
    harness.checkpoint()
