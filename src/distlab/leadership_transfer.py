...
class LeadershipTransfer:
        ...
        *,
        ...
        for timeout_attempt in range(max_timeout_now_attempts):
            ...
            try:
                ...
                if target_unavailable:
                    raise LeadershipTransferTargetUnavailable(
                        f"leadership transferee {transferee_id!r} crashed before TimeoutNow retry dispatch"
                    ) from exc
                raise LeadershipTransferIncomplete(
        ...
        if not self._transfer_elected(target, previous_term):
            ...
            if source_lost_authority and transferee_did_not_start_next_term:
                ...
                raise LeadershipTransferIncomplete(
                    "source leader lost leadership while waiting for transferee election"
                )
            self._record_failure(
                transferee_id,
                previous_term,
    ...
    def _transfer_elected(self, target: RaftNode, previous_term: int) -> bool:
        source_retired = not self.sim.is_alive(self.leader.node_id) or (
            self.leader.role is not RaftRole.LEADER
            and (
                self.leader.role is RaftRole.FOLLOWER
                or self.leader.current_term >= target.current_term
            )
        )
        return (
